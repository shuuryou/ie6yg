...
Public Function Base64Encode2(InData() As Byte, ByVal InLen As Long) As String
  ...
  Dim ODataLen As Long
    ODataLen = (InLen * 4 + 2) \ 3     ' output length without padding
  Dim OLen As Long
    OLen = ((InLen + 2) \ 3) * 4           ' output length including padding
  Dim Out() As Byte
  ...
  Dim o3 As Byte
        ...
        Out(op) = Map1(o1)
        op = op + 1
        Out(op) = IIf(op < ODataLen, Map1(o2), Asc("="))
        op = op + 1
        Out(op) = IIf(op < ODataLen, Map1(o3), Asc("="))
        op = op + 1
    Loop
...
Public Function Base64Decode(ByVal s As String) As Byte()
  ...
  Dim i3 As Byte
        If ip < ILen Then i3 = IBuf(ip): ip = ip + 1 Else i3 = Asc("A")
...
':) Ulli's VB Code Formatter V2.24.17 (2022-Nov-16 03:43)  Decl: 26  Code: 223  Total: 249 Lines
':) CommentOnly: 43 (17.3%)  Commented: 2 (0.8%)  Filled: 209 (83.9%)  Empty: 40 (16.1%)  Max Logic Depth: 2
